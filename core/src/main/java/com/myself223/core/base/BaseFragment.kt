...
abstract class BaseFragment<Binding : ViewBinding,ViewModel : BaseViewModel>(
        ...
        safeFlowGather(lifecycleState) {
            collect{
                ...
                when(it){
                    ...
                    is UiState.Error->error?.invoke(it.message)
                }
    ...
    fun <T>Flow<Either<String,T>>.safeFlowGather(
        actionIfEitjerRight  :suspend(T)-> Unit,
        actionIfEitjerLeft  :(error:String)-> Unit,
    ){
        safeFlowGather {
            collect{
                when(it){
                    is Either.Left   -> actionIfEitjerLeft(it.eshkere)
                    is Either.Right   -> actionIfEitjerRight(it.eshkere)
